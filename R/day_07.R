...
#' Find the parent of a given bag
#'
#' @param colors colors to search for
#'
#' @return  a vector with paren colors
find_parent <- function( colors){
  limit <- cleaned_data[(cleaned_data$child_color) %in% colors, ]
  limit$parent

}

#' count the number of possible parents for a given bag
#'
#' @param map = mapping data frame,
#' @param  color color to search for.
#' @param n number of searches to make
#'
#' @return  a number the total number of parent bags
#'
num_parents <- function(map, color, n=5000){

  listed <- purrr::accumulate(1:n, find_parent, .init = color)

  length(unique(unlist(listed)))-1
}
...
text <- readLines("input/day_07.txt")
...
data %>%
  ...
  separate_rows(child,
                sep = ", ") %>%
  extract(child,
          c("child_count", "child_color"),
          "([\\d]*) (.*) bag",
          convert = TRUE,
          remove = FALSE) %>%
  filter(child != "No Other bags")-> cleaned_data




num_parents(map = cleaned_data, "shiny gold", 2)

find_parent(cleaned_data, "shiny gold")
...
num_contained <- function(map,  colors, parent = "parent", child = "child_color", count="child_count" ){

 limited <- map[map[[parent]] == colors, ]
 names(limited)
  print(unlist(colors))
 names <- limited[[child]]
...
# Problem 2 Testing ===========================================================
num_contained(cleaned_data, "shiny gold")
