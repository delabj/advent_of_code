...
# This seems like it will be easier to do as a pipeline. It's just fining the
# difference making sure they're all within 1 to 3 and then taking the product
library(tidyverse)

adapter_lines <-  as.numeric(readLines("input/day_10.txt"))

data = data.frame(
  adapters = c(
    0,  # jolts from source
    adapter_lines, # adapters in bag
    max(adapter_lines)+3 # maximum range
   )) %>%
  arrange(adapters)
...
data %>%
  mutate(difference = adapters-lag(adapters)) %>%
  group_by(difference) %>%
  count() %>%
  ungroup() %>%
  drop_na() %>%
  summarise(product = prod(n))
...
#' So I think this is going to be doable without a function today. Values that
#' are 3 apart are contently fixed which should reduce our total number to check.
#' Meaning that we can treat them like a single block.
#'
#' We can also do this for other numbers that only have a single pair of
#' members or a single member
#'
#' Using the Tribonacci numbers (signature (0,0,1)) We can see that
#' Groups of 3 can be arranged in 2 different ways
#' Groups of 4 can be arranged in 4 ways
#' Groups of 5 can be arranged in 7 ways
#'


data.frame(adapters=data[-c(94),]) %>%
  mutate(one_group= if_else(adapters+1 == lead(adapters), 0, 1)) %>%
  mutate(cum_sum = cumsum(one_group)) %>%
  drop_na() %>%
  add_row(adapters =0, one_group = 0, cum_sum = 0, .before = 1)%>%
  group_by(cum_sum) %>%
  count() %>%
  ungroup() %>%
  group_by(n) %>%
  ...
  mutate(
    ...
    summarise( product = prod(combos_added))
