...
#' No Operation
#'
#' @description Move to the next function
#'
#' @param value The value to work with
#' @param adjust the adjusment,
#' @param index of current function
#' @param functions A list of functions
#' @param fun_values a list of values for the functions
#' @param visited a list of indexces visited.
func_nop <- function(value, adjust, index, functions,fun_values,  visited){

  next_index = index + 1

  if(next_index %in% visited){
    usethis::ui_warn("Looped")
    return(value)
    }

  next_value    <- value
  next_adjust   <- fun_values[next_index]
  next_visited  <- c(visited, index)
  print(instructions[index])


  fun <- match.fun(paste0("func_",functions[next_index]))
...
#' @param fun_values a list of values for the functions
#' @param visited a list of indexces visited.
func_acc <- function(value, adjust, index, functions,fun_values,  visited){
...
#' @param fun_values a list of values for the functions
#' @param visited a list of indexces visited.
func_jmp <- function(value, adjust, index, functions,fun_values,  visited){
  next_index = index + adjust
...
# Problem 1 Testing ===========================================================
library(tidyverse)
instructions <- readLines("input/day_08.txt")

instructions_split <- data.frame(raw= instructions) %>%
  separate(col = raw, into = c("func", "value"), sep = " ")%>%
...
func_acc(
  ...
  adjust = 40,
  index = 1,
  functions = instructions_split$func,
  fun_values = instructions_split$value,
  visited = c()
  )
...
instructions2 <- readLines("input/day_08-fixed.txt")


instructions2_split <- data.frame(raw= instructions2) %>%
  separate(col = raw, into = c("func", "value"), sep = " ")%>%
  mutate(id = row_number(),
         value = as.numeric(
           str_remove(value, "[+]")
...
func_acc(
  ...
  adjust = 40,
  index = 1,
  functions = instructions2_split$func,
  fun_values = instructions2_split$value,
  visited = c()
)
